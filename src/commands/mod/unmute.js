...
module.exports = class Unmute extends Interaction {
    constructor() {
        super({
            name: "unmute",
    ...
    async exec(int, data) {
        ...
        if(int.member.roles.highest.position < member.roles.highest.position) {
            return int.reply({
                content: "You can't unmute someone with a higher role than you!",
                ephemeral: true,
            });
        }
        ...
        await member.voice.setMute(false, reason).then(async () => {
            if(data.modLogs) {
            ...
            return int.reply({
                content: `${member} has been unmuted! Reason: **${reason}**`,
                ephemeral: true,
            });
        });
